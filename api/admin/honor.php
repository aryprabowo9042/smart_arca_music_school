<?php
// --- 1. MESIN LOGOUT INTERNAL (ANTI ERROR 403) ---
if (isset($_GET['action']) && $_GET['action'] == 'logout') {
    setcookie('user_role', '', time() - 3600, '/');
    setcookie('user_id', '', time() - 3600, '/');
    setcookie('user_username', '', time() - 3600, '/');
    header("Location: ../../index.php"); // Mundur ke root
    exit();
}
...
if (isset($_POST['simpan_transaksi'])) {
    ...
    $tip = $_POST['jenis']; 
    $nom = (int)$_POST['nominal']; // Pastikan dibulatkan jadi integer
...
// Data Honor Guru (50%) - Gunakan FLOOR agar bulat ke bawah
...
// Inisialisasi Data Edit (Anti-Warning)
    ...
    <div class="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            ...
            <form method="POST" onsubmit="return confirm('PERINGATAN! Ini akan menghapus SEMUA riwayat keuangan & absensi. Saldo akan kembali ke 0. Lanjutkan?')">
                <button type="submit" name="reset_total" class="w-full bg-slate-900 text-white py-4 rounded-[2rem] text-[10px] font-bold uppercase tracking-widest hover:bg-black transition shadow-xl">
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-xl border-t-8 border-yellow-400" id="formArea">
                ...
                <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-1">
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Jenis Kas</label>
                    ...
                    <div class="md:col-span-1">
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Nama Pelaku</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Keterangan</label>
                        <input type="text" name="keterangan" id="keterangan" value="<?php echo htmlspecialchars($edit_data['keterangan']); ?>" class="w-full p-3 border-2 border-slate-50 rounded-xl bg-slate-50 text-sm font-bold focus:border-red-600 outline-none" placeholder="Tujuan transaksi..." required>
                    </div>

                    <div class="md:col-span-2">
                        <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Nominal (Rp)</label>
                        <input type="number" name="nominal" id="nominal" value="<?php echo (int)$edit_data['nominal']; ?>" class="w-full p-4 border-2 border-slate-50 rounded-2xl bg-slate-50 font-black text-3xl text-red-600 focus:border-red-600 outline-none" required>
                    </div>

                    <input type="hidden" name="tanggal" value="<?php echo $edit_data['tanggal']; ?>">
                    <button type="submit" name="simpan_transaksi" class="md:col-span-2 bg-red-600 hover:bg-red-700 text-yellow-400 font-black py-4 rounded-2xl shadow-xl transition active:scale-95 uppercase text-xs tracking-widest">Simpan Transaksi</button>
                </form>
            </div>

            <div class="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100">
                <div class="p-6 bg-slate-50 border-b">
                    <h3 class="font-black text-slate-800 uppercase text-xs italic tracking-widest leading-none">Arus Kas Terakhir</h3>
                </div>
