...
if (isset($_POST['simpan_transaksi'])) {
    ...
    $tip = $_POST['jenis']; 
    $nom = (int)$_POST['nominal']; // DIPAKSA JADI BULAT
...
// FIX PHP WARNING: Inisialisasi variabel agar tidak NULL
...
// ==========================================
// 2. PERHITUNGAN SALDO (PASTI BULAT)
// ==========================================
$q_spp = mysqli_query($conn, "SELECT SUM(CAST(nominal_bayar AS UNSIGNED)) as total FROM absensi");
$total_spp = (int)(mysqli_fetch_assoc($q_spp)['total'] ?? 0);

$q_masuk = mysqli_query($conn, "SELECT SUM(CAST(nominal AS UNSIGNED)) as total FROM keuangan WHERE jenis='masuk' AND status_konfirmasi = 1");
$total_masuk = (int)(mysqli_fetch_assoc($q_masuk)['total'] ?? 0);

$q_keluar = mysqli_query($conn, "SELECT SUM(CAST(nominal AS UNSIGNED)) as total FROM keuangan WHERE jenis='keluar' AND status_konfirmasi = 1");
$total_keluar = (int)(mysqli_fetch_assoc($q_keluar)['total'] ?? 0);
...
<body class="bg-slate-50 min-h-screen pb-10">
    ...
    <nav class="bg-red-600 shadow-lg px-6 py-4 flex justify-between items-center mb-8 sticky top-0 z-50 border-b-4 border-yellow-400">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-white font-black text-xl leading-none tracking-tighter uppercase italic">Smart Arca</h1>
    ...
    <div class="max-w-6xl mx-auto px-4 space-y-8">
        ...
        <div class="bg-white border-2 border-red-600 p-6 rounded-[2rem] shadow-2xl flex justify-between items-center mb-4 relative overflow-hidden">
            <div class="absolute top-0 right-0 bg-red-600 text-yellow-400 px-4 py-1 text-[10px] font-black rounded-bl-xl">PENDING</div>
            <div>
            ...
            <a href="honor.php?konfirmasi_cair=<?php echo $p['id']; ?>" class="bg-red-600 text-yellow-400 px-6 py-3 rounded-2xl text-xs font-black shadow-lg hover:scale-105 transition">KONFIRMASI</a>
        </div>
        <?php endwhile; ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-[2rem] shadow-xl border-t-8 border-red-600">
                    <h3 class="font-black text-slate-800 mb-6 uppercase text-xs tracking-widest border-b-2 border-yellow-400 pb-2">Hak Guru (50%)</h3>
                    <?php while($g = mysqli_fetch_assoc($list_guru)): ?>
                    <div class="flex justify-between items-center border-b border-dashed py-3 last:border-0">
                        <div>
                            <p class="font-black text-sm text-slate-700 uppercase italic"><?php echo $g['username']; ?></p>
                            <p class="text-[10px] text-green-600 font-bold">Saldo: Rp <?php echo number_format((int)$g['hak_honor']); ?></p>
                        </div>
                        <button onclick="document.getElementById('nama_pelaku').value='<?php echo $g['username']; ?>'; document.getElementById('nominal').value='<?php echo (int)$g['hak_honor']; ?>'; document.getElementById('keterangan').value='Pembayaran Honor'; document.getElementById('jenis_transaksi').value='keluar'; document.getElementById('nominal').focus();" class="bg-red-50 text-red-600 px-3 py-1 rounded-lg text-[10px] font-black hover:bg-red-600 hover:text-white transition">BAYAR</button>
                    </div>
                    <?php endwhile; ?>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-white p-8 rounded-[2rem] shadow-xl border-t-8 border-yellow-400" id="formArea">
                    <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <input type="number" name="nominal" id="nominal" value="<?php echo (int)$edit_data['nominal']; ?>" placeholder="Nominal" class="p-4 border-2 border-slate-100 rounded-xl md:col-span-2 font-black text-2xl text-red-600 focus:border-red-500 outline-none" required>
                        <input type="hidden" name="tanggal" value="<?php echo $edit_data['tanggal']; ?>">
                        <button type="submit" name="simpan_transaksi" class="md:col-span-2 bg-red-600 text-yellow-400 font-black py-4 rounded-2xl shadow-xl hover:bg-red-700 transition uppercase tracking-widest">Simpan Transaksi</button>
                    </form>
                </div>

                <div class="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-slate-100">
                    <div class="p-6 bg-slate-50 border-b font-black text-slate-800 text-sm italic uppercase tracking-widest">Riwayat Kas Terkini</div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-slate-50">
                                <?php 
                                // AMBIL DATA DARI KEUANGAN MANUAL
                                $sql_history = "
                                    (SELECT tanggal, nama_pelaku, keterangan, jenis, nominal, id, 'manual' as sumber FROM keuangan WHERE status_konfirmasi = 1)
                                    UNION
                                    (SELECT a.tanggal, u.username as nama_pelaku, CONCAT('Bayar SPP: ', j.alat_musik) as keterangan, 'masuk' as jenis, a.nominal_bayar as nominal, a.id, 'spp' as sumber 
                                     FROM absensi a JOIN jadwal j ON a.id_jadwal = j.id JOIN users u ON j.id_murid = u.id)
                                ...
                                <tr class="hover:bg-slate-50 transition">
                                    ...
                                    <td class="p-5">
                                        ...
                                        <p class="text-[10px] text-slate-400"><?php echo htmlspecialchars($r['keterangan']); ?> 
                                           <?php if($r['sumber'] == 'spp') echo '<span class="text-[8px] bg-green-100 text-green-600 px-1 rounded">SPP</span>'; ?>
                                        </p>
                                    </td>
