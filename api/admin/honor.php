...
// D. AMBIL DATA UNTUK FORM EDIT (Jika tombol edit diklik)
$data_edit = null;
...
<body class="bg-slate-50 min-h-screen pb-20">
    ...
    <nav class="bg-red-800 text-white py-4 px-6 shadow-xl border-b-4 border-yellow-400 sticky top-0 z-50 flex justify-between items-center">
        <div class="flex items-center gap-3">
            ...
            <h1 class="font-black italic uppercase tracking-tighter">Finance Center</h1>
        </div>
        <div class="bg-red-900 px-4 py-1 rounded-full border border-red-700 text-[10px] font-black uppercase">
            Saldo: Rp <?php echo number_format($saldo_bersih, 0, ',', '.'); ?>
        </div>
    </nav>
    ...
    <div class="max-w-7xl mx-auto p-6 md:p-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            ...
            <div class="bg-indigo-900 p-8 rounded-[2.5rem] shadow-xl text-white flex justify-between items-center relative overflow-hidden">
                ...
                <i class="fas fa-vault text-6xl text-white/10 absolute right-4 -bottom-2"></i>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white p-8 rounded-[3rem] shadow-2xl border-2 border-red-700 sticky top-28">
                    <h2 class="text-xl font-black text-slate-800 uppercase italic mb-6 leading-none border-b-2 border-yellow-400 pb-2">
                        <?php echo $data_edit ? 'Edit Transaksi' : 'Input Transaksi'; ?>
                    </h2>
                    ...
                    <form method="POST" class="space-y-4 text-[10px] font-black uppercase tracking-widest">
                        ...
                        <div>
                            ...
                            <input type="date" name="tgl" value="<?php echo $data_edit['tanggal'] ?? date('Y-m-d'); ?>" class="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-50 focus:border-red-600 outline-none font-bold" required>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="ml-2 text-slate-400 italic">Kategori</label>
                            <input type="text" name="kategori" value="<?php echo $data_edit['kategori'] ?? ''; ?>" class="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-50 outline-none font-bold" placeholder="Misal: Uang Pendaftaran" required>
                        </div>
                        <div>
                            <label class="ml-2 text-slate-400 italic">Nominal (Rp)</label>
                            <input type="number" name="nominal" value="<?php echo $data_edit['nominal'] ?? ''; ?>" class="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-50 outline-none font-bold" placeholder="Nominal" required>
                        </div>
                        <div>
                            <label class="ml-2 text-slate-400 italic">Keterangan</label>
                            <textarea name="keterangan" class="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-50 outline-none font-bold"><?php echo $data_edit['keterangan'] ?? ''; ?></textarea>
                        </div>
                        
                        <button type="submit" name="simpan_transaksi" class="w-full bg-red-700 text-white py-4 rounded-2xl font-black uppercase shadow-xl hover:bg-red-800 transition transform active:scale-95 italic text-xs tracking-widest">
                            <?php echo $data_edit ? 'Update Data' : 'Simpan Data'; ?> <i class="fas fa-save ml-2"></i>
                        </button>
                        
                        <?php if($data_edit): ?>
                            <a href="honor.php" class="block text-center text-slate-400 font-black text-[9px] uppercase mt-2 hover:text-red-600 transition">Batal Edit</a>
                        <?php endif; ?>
                    </form>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-12">
                ...
                <section>
                    <h2 class="text-xl font-black text-slate-800 uppercase italic mb-6 border-b-2 border-red-700 pb-2 inline-block"><i class="fas fa-hand-holding-usd mr-2 text-red-700"></i> Pengajuan Honor Guru</h2>
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        if(mysqli_num_rows($q_req) == 0) echo '<div class="p-8 bg-white rounded-3xl text-center text-slate-300 font-bold italic border-2 border-dashed">Belum ada pengajuan.</div>';
                        ...
                        <div class="bg-white p-6 rounded-[2rem] shadow-lg flex justify-between items-center border-l-[10px] border-red-700">
                            <div>
                                ...
                                <p class="text-[10px] font-black text-red-600 italic mt-1 uppercase">Hak: Rp <?php echo number_format($row['total']*0.5, 0, ',', '.'); ?></p>
                            </div>
                            <form method="POST">
                                <input type="hidden" name="id_guru" value="<?php echo $row['id_guru']; ?>">
                                <button type="submit" name="bayar_honor" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg transition">Konfirmasi Lunas</button>
                            </form>
                        </div>
                        <?php endwhile; ?>
                    </div>
                            ...
                            <tbody class="font-bold text-slate-600 italic">
                                ...
                                <tr class="border-b border-slate-50 hover:bg-slate-50 transition">
                                    ...
                                    <td class="p-5 text-center flex justify-center gap-3">
                                        <a href="honor.php?edit_id=<?php echo $m['id']; ?>" class="text-indigo-400 hover:text-indigo-700 transition" title="Edit Data"><i class="fas fa-edit"></i></a>
                                        <a href="honor.php?hapus_id=<?php echo $m['id']; ?>" class="text-red-300 hover:text-red-600 transition" onclick="return confirm('Hapus data ini?')" title="Hapus Data"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
