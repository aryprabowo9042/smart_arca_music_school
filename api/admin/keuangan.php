...
// A. Ambil Data SPP Les (Otomatis dari Jurnal Guru)
...
// B. Ambil Data Honor Guru (Pengeluaran 50% dari yang statusnya Lunas)
$sql_honor = "SELECT a.tanggal, u.username as keterangan, 'pengeluaran' as jenis, (a.nominal_bayar * 0.5) as nominal, 'Honor Guru' as kategori
              FROM absensi a 
              JOIN jadwal j ON a.id_jadwal = j.id 
              JOIN users u ON j.id_guru = u.id 
              WHERE a.status_honor = 'selesai' AND MONTH(a.tanggal) = '$bulan_pilih' AND YEAR(a.tanggal) = '$tahun_pilih'";

// C. Ambil Data Transaksi Manual (Pendaftaran, Listrik, Sewa, dll)
$sql_manual = "SELECT tanggal, kategori as keterangan, jenis, nominal, 'Operasional/Lain' as kategori
               FROM transaksi_manual 
               WHERE MONTH(tanggal) = '$bulan_pilih' AND YEAR(tanggal) = '$tahun_pilih'";

// D. Gabungkan Semua Data Menggunakan UNION dan Urutkan Berdasarkan Tanggal
$sql_gabungan = "($sql_spp) UNION ($sql_honor) UNION ($sql_manual) ORDER BY tanggal ASC";
...
<body class="bg-slate-50 min-h-screen pb-20">
    ...
    <nav class="bg-indigo-900 text-white py-4 px-6 shadow-xl border-b-4 border-yellow-400 sticky top-0 z-50 flex justify-between items-center no-print">
        ...
        <button onclick="window.print()" class="bg-yellow-400 text-indigo-900 px-4 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">
    ...
    <div class="max-w-6xl mx-auto p-6 md:p-10">
        ...
        <div class="bg-white p-6 rounded-[2rem] shadow-md mb-10 no-print border-2 border-slate-100">
            <form method="GET" class="flex flex-wrap items-end gap-6">
                <div class="flex-1 min-w-[150px]">
                    ...
                    <select name="bulan" class="w-full p-3 rounded-xl bg-slate-50 border-2 border-slate-50 outline-none font-bold italic">
                ...
                <div class="flex-1 min-w-[150px]">
                    ...
                    <select name="tahun" class="w-full p-3 rounded-xl bg-slate-50 border-2 border-slate-50 outline-none font-bold italic">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            ...
            <div class="bg-indigo-900 p-8 rounded-[2.5rem] shadow-xl text-white">
                ...
                <h3 class="text-2xl font-black text-yellow-400 italic">Rp <?php echo number_format($laba_bersih, 0, ',', '.'); ?></h3>
            </div>
        </div>
            ...
            <table class="w-full text-left text-xs">
                ...
                <tbody class="font-bold text-slate-600">
                    ...
                    <tr class="border-b border-slate-50 hover:bg-slate-50 transition">
                        ...
                        <td class="p-6"><span class="bg-slate-100 px-3 py-1 rounded-full text-[9px] uppercase"><?php echo $row['kategori']; ?></span></td>
                        <td class="p-6 font-black text-green-600"><?php echo ($row['jenis'] == 'pemasukan') ? number_format($row['nominal'], 0, ',', '.') : '-'; ?></td>
                        <td class="p-6 font-black text-red-600"><?php echo ($row['jenis'] == 'pengeluaran') ? number_format($row['nominal'], 0, ',', '.') : '-'; ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
