...
<body class="bg-white" x-data="{ 
    ...
    <nav class="bg-red-700 text-white py-4 px-6 shadow-xl border-b-4 border-yellow-400 sticky top-0 z-50">
        <div class="max-w-6xl mx-auto flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <h1 class="font-black italic uppercase text-lg tracking-tighter">Smart Arca</h1>
            </div>
    ...
    <header class="py-20 px-6 bg-gradient-to-b from-red-50 to-white text-center">
        <div class="max-w-4xl mx-auto">
            <h1 class="text-5xl md:text-7xl font-black text-slate-900 italic uppercase tracking-tighter mb-6">Wujudkan <span class="text-red-600">Mimpimu</span></h1>
            ...
            <div class="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-2xl mx-auto">
                ...
                <div class="bg-white p-6 rounded-[2.5rem] shadow-xl border-l-8 border-yellow-400">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Admin (Mbak Fia):</p>
                    <a href="https://wa.me/62895360796038" target="_blank" class="text-red-700 font-black italic flex items-center gap-2 text-sm uppercase"><i class="fab fa-whatsapp text-green-500 text-xl"></i> +62 895-3607-96038</a>
                </div>
            </div>
        </div>
    </header>
    ...
    <template x-if="openModal">
        <div class="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm" @click.self="openModal = null">
            <div class="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden">
                <div class="bg-red-700 p-6 text-white flex justify-between border-b-4 border-yellow-400">
                    <h3 class="font-black italic uppercase" x-text="'Silabus ' + openModal"></h3>
                ...
                <div class="p-6 bg-white text-center"><a href="https://wa.me/62895360796038" target="_blank" class="bg-red-700 text-white px-8 py-3 rounded-2xl font-black uppercase text-[10px] shadow-lg italic">Daftar Sekarang</a></div>
            </div>
        </div>
    </template>
    ...
    <div class="fixed bottom-6 right-6 z-[100]">
        ...
        <div x-show="chatOpen" x-cloak x-transition class="absolute bottom-20 right-0 w-[350px] bg-white rounded-[2rem] shadow-2xl border-b-8 border-yellow-400 flex flex-col overflow-hidden">
            <div class="bg-red-700 p-4 text-white flex items-center gap-3">
                <i class="fas fa-robot text-yellow-400"></i>
                <p class="font-black italic text-xs uppercase">Arca AI Assistant</p>
            </div>
            <div class="h-[300px] overflow-y-auto p-4 space-y-4 bg-slate-50 text-[11px]" id="chat-box">
                <template x-for="msg in messages">
                    <div :class="msg.role === 'user' ? 'flex justify-end' : 'flex justify-start'">
                        <div :class="msg.role === 'user' ? 'bg-red-600 text-white' : 'bg-white border text-slate-700'" class="p-3 max-w-[80%] font-bold italic rounded-2xl shadow-sm" x-text="msg.text"></div>
                    </div>
                </template>
                <div x-show="loading" class="text-[9px] font-black uppercase text-slate-400 animate-pulse">Arca sedang mengetik...</div>
            </div>
            <div class="p-4 border-t bg-white flex gap-2">
                <input type="text" x-model="userInput" @keyup.enter="sendMessage()" placeholder="Tanya apa saja..." class="flex-1 bg-slate-100 p-3 rounded-xl outline-none text-xs font-bold italic">
                <button @click="sendMessage()" class="bg-red-700 text-yellow-400 w-10 h-10 rounded-xl"><i class="fas fa-paper-plane"></i></button>
            </div>
        </div>
    </div>
    ...
    <section id="login-section" class="py-20 bg-slate-100">
        <div class="max-w-md mx-auto px-6 bg-white p-10 rounded-[3rem] shadow-2xl border-b-8 border-yellow-400 text-center">
            ...
            <form method="POST" class="space-y-4">
                <input type="text" name="username" placeholder="Username" class="w-full p-4 rounded-2xl bg-slate-50 border-2 font-bold italic">
                <input type="password" name="password" placeholder="Password" class="w-full p-4 rounded-2xl bg-slate-50 border-2 font-bold">
                <button type="submit" name="login" class="w-full bg-red-600 text-white font-black py-4 rounded-2xl uppercase">Masuk Dashboard</button>
            </form>
        </div>
    </section>
    <?php endif; ?>
    ...
    <script>
        function sendMessage() {
            ...
            const txt = data.userInput;
            data.messages.push({ role: 'user', text: txt });
            data.userInput = ''; data.loading = true;
            setTimeout(() => { document.getElementById('chat-box').scrollTop = 10000; }, 50);

            // ALAMAT FETCH DIPERBAIKI UNTUK VERCEL
            fetch('api/chat.php', {
                method: 'POST',
            ...
            .then(res => res.json())
            .then(res => { data.messages.push({ role: 'bot', text: res.reply }); })
            .catch(() => { data.messages.push({ role: 'bot', text: 'Koneksi terputus.' }); })
            .finally(() => { 
                data.loading = false;
                setTimeout(() => { document.getElementById('chat-box').scrollTop = 10000; }, 50);
            });
